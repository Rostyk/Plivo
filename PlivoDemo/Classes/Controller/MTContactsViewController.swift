...
open class MTContactsViewController: UITableViewController, UISearchResultsUpdating, UISearchBarDelegate {

    // MARK: - Properties

    internal var contactDelegate: MTPickerDelegate?
    var contactsStore: CNContactStore?
    var resultSearchController = UISearchController()
    var orderedContacts = [String: [CNContact]]() //Contacts ordered in dicitonary alphabetically
    var sortedContactKeys = [String]()

    var selectedContacts = [MTContact]()
    ...
    override open func viewDidLoad() {
        super.viewDidLoad()
        self.title = MTGlobalConstants.Strings.contactsTitle
    ...
    fileprivate func registerContactCell() {
        ...
        if let bundleURL = podBundle.url(forResource: MTGlobalConstants.Strings.bundleIdentifier, withExtension: "bundle") {

            if let bundle = Bundle(url: bundleURL) {

                let cellNib = UINib(nibName: MTGlobalConstants.Strings.cellNibIdentifier, bundle: bundle)
            ...
            let cellNib = UINib(nibName: MTGlobalConstants.Strings.cellNibIdentifier, bundle: nil)
            tableView.register(cellNib, forCellReuseIdentifier: "Cell")
        }
    }
    ...
    convenience internal init(delegate: MTPickerDelegate?) {
        self.init(delegate: delegate, multiSelection: false)
    }

    convenience internal init(delegate: MTPickerDelegate?, multiSelection: Bool) {
        self.init(style: .plain)
        self.multiSelectEnabled = multiSelection
        contactDelegate = delegate
    }

    convenience internal init(delegate: MTPickerDelegate?, multiSelection: Bool, subtitleCellType: SubtitleCellValue) {
        self.init(style: .plain)
        self.multiSelectEnabled = multiSelection
        contactDelegate = delegate
    ...
    func getContacts(_ completion:  @escaping ContactsHandler) {
        ...
        let error = NSError(domain: "MTContactPickerErrorDomain", code: 1, userInfo: [NSLocalizedDescriptionKey: "No Contacts Access"])
    ...
    override open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! MTContactCell
        cell.accessoryType = UITableViewCellAccessoryType.none
        cell.selectionStyle = .blue
        //Convert CNContact to MTContact
		let contact: MTContact

        if resultSearchController.isActive {
            contact = MTContact(contact: filteredContacts[(indexPath as NSIndexPath).row])
        } else {
			...
			contact = MTContact(contact: contactsForSection[(indexPath as NSIndexPath).row])
        }
    ...
    override open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let cell = tableView.cellForRow(at: indexPath) as! MTContactCell
    ...
    func onTouchCancelButton() {
        contactDelegate?.epContactPicker(self, didCancel: NSError(domain: "MTContactPickerErrorDomain", code: 2, userInfo: [ NSLocalizedDescriptionKey: "User Canceled Selection"]))
